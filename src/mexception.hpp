...
struct is_one_of<T, First, Rest...> {
    static constexpr bool value = std::is_same_v<T, First> || is_one_of<T, Rest...>::value;
};
...
const T &get_or_throw(const std::variant<Ts...> &v) {
  if constexpr (is_one_of<T, Ts...>::value) {
    ...
    throw std::runtime_error("Variant does not currently hold the requested type: " + std::string(typeid(T).name()));
  } else {
    // If T is not a possible type for this variant.
    throw std::runtime_error("Requested type " + std::string(typeid(T).name()) + " is not a valid alternative for this variant");
  }
}
